// Quality Auto Care - Interactive Scripts
...
function checkAutoScroll() {
    if (window.innerWidth < 768) { if (!autoScrollInterval) startAutoScroll(); }
    else { stopAutoScroll(); }
}
...
contactForm.addEventListener('submit', function(e) {
    ...
    window.location.href = `mailto:info@qualityautocare.com?subject=${subject}&body=${body}`;
...
setTimeout(() => {
    document.querySelectorAll('.hero-ctas .btn').forEach(btn => { btn.classList.add('btn-pulse'); });
}, 3000);
...
document.querySelectorAll('a[href^="tel:"]').forEach(link => {
    link.addEventListener('click', () => {
        if (typeof gtag === 'function') { gtag('event', 'click_to_call', { business: 'Quality Auto Care' }); }
    });
});

window.addEventListener('load', () => {
    const heroBadge = document.querySelector('.hero-badge');
    if (heroBadge) { heroBadge.style.opacity = '0'; heroBadge.style.transform = 'translateY(20px)'; setTimeout(() => { heroBadge.style.transition = 'all 0.6s ease'; heroBadge.style.opacity = '1'; heroBadge.style.transform = 'translateY(0)'; }, 200); }
    const heroH1 = document.querySelector('.hero h1');
    if (heroH1) { heroH1.style.opacity = '0'; heroH1.style.transform = 'translateY(20px)'; setTimeout(() => { heroH1.style.transition = 'all 0.6s ease'; heroH1.style.opacity = '1'; heroH1.style.transform = 'translateY(0)'; }, 400); }
    const heroSub = document.querySelector('.hero-sub');
    if (heroSub) { heroSub.style.opacity = '0'; heroSub.style.transform = 'translateY(20px)'; setTimeout(() => { heroSub.style.transition = 'all 0.6s ease'; heroSub.style.opacity = '1'; heroSub.style.transform = 'translateY(0)'; }, 600); }
    const heroCtas = document.querySelector('.hero-ctas');
    if (heroCtas) { heroCtas.style.opacity = '0'; heroCtas.style.transform = 'translateY(20px)'; setTimeout(() => { heroCtas.style.transition = 'all 0.6s ease'; heroCtas.style.opacity = '1'; heroCtas.style.transform = 'translateY(0)'; }, 800); }
    const heroTrust = document.querySelector('.hero-trust');
    if (heroTrust) { heroTrust.style.opacity = '0'; heroTrust.style.transform = 'translateY(20px)'; setTimeout(() => { heroTrust.style.transition = 'all 0.6s ease'; heroTrust.style.opacity = '1'; heroTrust.style.transform = 'translateY(0)'; }, 1000); }
